...
if [ "${CONNECT_OR_DISCONNECT}" -eq 1 ]; then
  CONN_TYPE="connected to"
else
  CONN_TYPE="disconnected from"
fi
...
RESULT=$(curl -X GET "https://stats.allstarlink.org/api/stats/${THEIR_NODE}" | jq -r '.node.server.User_ID + "|" + .node.server.Location')
...
CALLSIGN_LOCATION=$(echo "$RESULT" | cut -d '|' -f 2)
...
CALLSIGN_DETAIL=$(curl -X GET "https://call3.n0agi.com/${CALLSIGN}/json/" | jq -r '.First + " " + .Last + "|" + ."License Status" + " " + ."Operator Class"')
CALLSIGN_NAME=$(echo "$CALLSIGN_DETAIL" | cut -d '|' -f 1)
CALLSIGN_STATUS=$(echo "$CALLSIGN_DETAIL" | cut -d '|' -f 2)
